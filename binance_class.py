...
class BinanceAPI:
    ...
    def __init__(self, p_api_pub_key=None, p_api_secret_key=None, p_symbol_first=None, p_symbol_second=None,
                 p_wallet=None):
    ...
    def build_client(self, p_api_pub_key=None, p_api_secret_key=None):
        ...
        try:
            if p_api_pub_key and p_api_secret_key:
                _temp = Client(api_key=p_api_pub_key, api_secret=p_api_secret_key,
                               requests_params={"timeout": self.request_timeout})
            else:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'build_client', _inputs, traceback.format_exc(2))}")
    ...
    def truncate_by_step_size(self, p_qta_start, p_step_size):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'truncate_by_step_size', _inputs, traceback.format_exc(2))}")
            return (_response_tuple)
    ...
    def general_check_if_symbol_exists(self, p_symbol_input=None):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_check_if_symbol_exists', _inputs, traceback.format_exc(2))}")
        ...
        try:
            ...
            if _exchange_info:
                ...
                if len(_rate_limits) > 0:
                    ...
                    _response_tuple = (
                    'NOK', f"{utility.my_log('Error', 'general_get_rate_limits', _inputs, '_rate_limits is None')}")

            else:
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'general_get_rate_limits', _inputs, '_exchange_info is None')}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_rate_limits', _inputs, traceback.format_exc(2))}")
    ...
    def general_get_system_status(self):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_system_status', _inputs, traceback.format_exc(2))}")
    ...
    def general_get_symbol_info_filter(self, p_what_filter, p_symbol_input=None):
        ...
        def price_filter(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    _output_local["PRICE_FILTER_minPrice"] = Decimal(_filter.get('minPrice')) if _filter.get(
                        'minPrice') else _filter.get('minPrice')
                    _output_local["PRICE_FILTER_maxPrice"] = Decimal(_filter.get('maxPrice')) if _filter.get(
                        'maxPrice') else _filter.get('maxPrice')
                    _output_local["PRICE_FILTER_tickSize"] = Decimal(_filter.get('tickSize')) if _filter.get(
                        'tickSize') else _filter.get('tickSize')
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.price_filter', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.price_filter', _inputs, traceback.format_exc(2))}")
            ...
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    if self.wallet == 'spot' or self.wallet == 'margin':
                        _output_local["PERCENT_PRICE_avgPriceMins"] = int(_filter.get('avgPriceMins')) if _filter.get(
                            'avgPriceMins') else _filter.get('avgPriceMins')
                    else:
                        _output_local["PERCENT_PRICE_multiplierDecimal"] = int(
                            _filter.get('multiplierDecimal')) if _filter.get('multiplierDecimal') else _filter.get(
                            'multiplierDecimal')
                    _output_local["PERCENT_PRICE_multiplierUp"] = Decimal(_filter.get('multiplierUp')) if _filter.get(
                        'multiplierUp') else _filter.get('multiplierUp')
                    _output_local["PERCENT_PRICE_multiplierDown"] = Decimal(
                        _filter.get('multiplierDown')) if _filter.get('multiplierDown') else _filter.get(
                        'multiplierDown')
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.percent_price', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.percent_price', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def lot_size(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    _output_local["LOT_SIZE_maxQty"] = Decimal(_filter.get('maxQty')) if _filter.get(
                        'maxQty') else _filter.get('maxQty')
                    _output_local["LOT_SIZE_minQty"] = Decimal(_filter.get('minQty')) if _filter.get(
                        'minQty') else _filter.get('minQty')  # quantity to buy or sell > symbol minQty
                    _output_local["LOT_SIZE_stepSize"] = Decimal(_filter.get('stepSize')) if _filter.get(
                        'stepSize') else _filter.get(
                        'stepSize')  # the quantity to buy or sell must be an exact multiple of symbol stepSize
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.lot_size', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.lot_size', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def min_notional(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin':
                    ...
                    _output_local["MIN_NOTIONAL_minNotional"] = Decimal(_filter.get('minNotional')) if _filter.get(
                        'minNotional') else _filter.get('minNotional')
                    _output_local["MIN_NOTIONAL_applyToMarket"] = _filter.get('applyToMarket')
                    _output_local["MIN_NOTIONAL_avgPriceMins"] = int(_filter.get('avgPriceMins')) if _filter.get(
                        'avgPriceMins') else _filter.get('avgPriceMins')
                    _response_tuple_local = ('OK', _output_local)
                elif self.wallet == 'futures':
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.min_notional', _inputs, self.wallet + ': attention! for this wallet min_notional does not exist ')}")
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.min_notional', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.min_notional', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def iceberg_parts(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin':
                    ...
                    _output_local["ICEBERG_PARTS_limit"] = int(_filter.get('limit')) if _filter.get(
                        'limit') else _filter.get('limit')
                    _response_tuple_local = ('OK', _output_local)
                elif self.wallet == 'futures':
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.iceberg_parts', _inputs, self.wallet + ': attention! for this wallet iceberg_parts does not exist ')}")
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.iceberg_parts', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.iceberg_parts', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def market_lot_size(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    _output_local["MARKET_LOT_SIZE_minQty"] = Decimal(_filter.get('minQty')) if _filter.get(
                        'minQty') else _filter.get('minQty')
                    _output_local["MARKET_LOT_SIZE_maxQty"] = Decimal(_filter.get('maxQty')) if _filter.get(
                        'maxQty') else _filter.get('maxQty')
                    _output_local["MARKET_LOT_SIZE_stepSize"] = Decimal(_filter.get('stepSize')) if _filter.get(
                        'stepSize') else _filter.get('stepSize')
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.market_lot_size', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.market_lot_size', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def max_num_algo_orders(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    if self.wallet == 'spot' or self.wallet == 'margin':
                        _output_local["MAX_NUM_ALGO_ORDERS_maxNumAlgoOrders"] = int(
                            _filter.get('maxNumAlgoOrders')) if _filter.get('maxNumAlgoOrders') else _filter.get(
                            'maxNumAlgoOrders')
                    else:
                        _output_local["MAX_NUM_ALGO_ORDERS_limit"] = int(_filter.get('limit')) if _filter.get(
                            'limit') else _filter.get('limit')
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.max_num_algo_orders', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.max_num_algo_orders', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        def max_num_orders(_symbol, _filter):
            try:
                if self.wallet == 'spot' or self.wallet == 'margin' or self.wallet == 'futures':
                    ...
                    if self.wallet == 'spot' or self.wallet == 'margin':
                        _output_local["MAX_NUM_ORDERS_maxNumOrders"] = int(_filter.get('maxNumOrders')) if _filter.get(
                            'maxNumOrders') else _filter.get('maxNumOrders')
                    else:
                        _output_local["MAX_NUM_ORDERS_limit"] = int(_filter.get('limit')) if _filter.get(
                            'limit') else _filter.get('limit')
                    _response_tuple_local = ('OK', _output_local)
                else:
                    _response_tuple_local = ('NOK',
                                             f"{utility.my_log('Error', 'general_get_symbol_info_filter.max_num_orders', _inputs, self.wallet + ': wallet is unknown')}")
            except Exception:
                _response_tuple_local = ('NOK',
                                         f"{utility.my_log('Exception', 'general_get_symbol_info_filter.max_num_orders', _inputs, traceback.format_exc(2))}")
            return (_response_tuple_local)
        ...
        try:
            if self.wallet == 'spot' or self.wallet == 'margin':
                ...
                if _symbol_info_dict['symbols']:
                    ...
                    _response_tuple = ('NOK',
                                       f"{utility.my_log('Error', 'general_get_symbol_info_filter', _inputs, _symbol_info_dict['symbols'] + ': _symbol_info_dict[symbols] is None')}")
                    return (_response_tuple)
            else:
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'general_get_symbol_info_filter', _inputs, self.wallet + ': wallet is unknown')}")
                return (_response_tuple)
        except BinanceAPIException as e:
            _error = str(e).split(":")[1]
            _response_tuple = ('NOK', _error)
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_symbol_info_filter', _inputs, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        if _symbol_info:
            ...
            if len(_filters) > 0:
                for _filter in _filters:
                    ...
                    if _filter.get('filterType') == p_what_filter:
                        ...
                        if p_what_filter == 'PRICE_FILTER':
                            ...
                            _response_tuple = ('NOK',
                                               f"{utility.my_log('Error', 'general_get_symbol_info_filter', _inputs, p_what_filter + ': what_filter is unknown')}")
                ...
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'general_get_symbol_info_filter', _inputs, '_filters is None')}")

        else:
            _response_tuple = (
            'NOK', f"{utility.my_log('Error', 'general_get_symbol_info_filter', _inputs, '_symbol_info is None')}")
    ...
    def general_get_symbol_fee_cost(self, p_what_fee='taker', p_symbol_input=None):
        ...
        try:
                        ...
                        for t in _trade_fee:
                            if t.get('symbol') == _symbol_work:
                                try:
                                    ...
                                    _response_tuple = ('NOK',
                                                       f"{utility.my_log('Exception', 'general_get_symbol_fee_cost', _inputs, traceback.format_exc(2))}")
                                    return (_response_tuple)
                    else:
                        _response_tuple = ('NOK',
                                           f"{utility.my_log('Error', 'general_get_symbol_fee_cost', _inputs, '_trade_fee is Empty')}")
                else:
                    _response_tuple = ('NOK',
                                       f"{utility.my_log('Error', 'general_get_symbol_fee_cost', _inputs, 'get_trade_fee() insuccess')}")
            else:
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'general_get_symbol_fee_cost', _inputs, '_trade_fee_response is None')}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_symbol_fee_cost', _inputs, traceback.format_exc(2))}")
    ...
    def general_get_symbol_avg_price(self, p_symbol_input=None):
        ...
        try:
            ...
            if _avg_price_response:
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'general_get_symbol_avg_price', _inputs, '_avg_price_response is None')}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_symbol_avg_price', _inputs, traceback.format_exc(2))}")
    ...
    def general_get_symbol_mark_price(self, p_symbol_input=None):
        ...
        try:
            ...
            if _response:
                ...
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'general_get_symbol_mark_price', _inputs, '_response is None')}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'general_get_symbol_mark_price', _inputs, traceback.format_exc(2))}")
    ...
    def account_get_balance_total(self):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_get_balance_total', _inputs, traceback.format_exc(2))}")
        ...
        if _what_finds in _account_info:
            ...
            for _what_find in _account_info[_what_finds]:
                ...
                if (Decimal(_what_find.get('free')) + Decimal(
                        _what_find.get('locked'))) > 0:  # Only Asset with something
            ...
            if self.wallet == 'futures' and _account_info:
                ...
                for _what_find in _account_info:
                    ...
                    if (Decimal(_what_find.get('balance'))) > 0:  # Only Asset with something
                        ...
                        _my_asset = {'asset': _what_find.get('asset'),
                                     ...
                                     'locked': Decimal(_what_find.get('balance')) - Decimal(
                                         _what_find.get('withdrawAvailable'))}
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'account_get_balance_total', _inputs, _what_finds + ' not in _account_info')}")
    ...
    def account_get_balance_asset_free(self, p_symbol):
        ...
        try:
            ...
            if self.wallet == 'spot':
                ...
                if _asset_balance_response:
                    if _asset_balance_response.get('free'):
                        ...
                        _response_tuple = ('NOK',
                                           f"{utility.my_log('Error', 'account_get_balance_asset_free', _inputs, '_asset_balance_response.get(free) is None')}")
                else:
                    _response_tuple = ('NOK',
                                       f"{utility.my_log('Error', 'account_get_balance_asset_free', _inputs, '_asset_balance_response is None')}")
                ...
                if _what_finds in _account_info:
                    ...
                    for _what_find in _account_info[_what_finds]:  # For every Balances
                        ...
                        if _what_find.get('asset') == p_symbol.upper():
                            ...
                            _response_tuple = ('NOK',
                                               f"{utility.my_log('Error', 'account_get_balance_asset_free', _inputs, p_symbol + ' not found')}")
                            continue
                ...
                if _assets_balance_response[0] == 'OK':
                    for _asset_balance_response in _assets_balance_response[1]:
                        if _asset_balance_response.get('asset') == p_symbol.upper():
                            ...
                            _response_tuple = ('NOK',
                                               f"{utility.my_log('Error', 'account_get_balance_asset_free', _inputs, p_symbol.upper() + ' not found')}")
                else:
                    _response_tuple = ('NOK',
                                       f"{utility.my_log('Error', 'account_get_balance_asset_free', _inputs, _assets_balance_response[1])}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_get_balance_asset_free', _inputs, traceback.format_exc(2))}")
    ...
    def account_get_quantity_to_buy(self, p_what_fee, p_type, p_size, p_how2get_qta2buy, p_price=None):
        ...
        try:
            ...
            _response_tuple = ('NOK',
                               f"{utility.my_log('Exception', 'account_get_quantity_to_buy', _inputs + _inputs_temp, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_get_quantity_to_buy', _inputs, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        if p_type == 'market':
            ...
            try:
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Exception', 'account_get_quantity_to_buy', _inputs, traceback.format_exc(2))}")
                return (_response_tuple)

        else:
            _response_tuple = ('NOK',
                               f"{utility.my_log('Error', 'account_get_quantity_to_buy', _inputs, 'p_type unknown or p_price is None')}")
            return (_response_tuple)
        ...
        quantity_post_stepSize_applied = self.truncate_by_step_size(quantity_pre_stepSize_applied,
                                                                    _symbol_step_size_value)
        ...
        if quantity_processed_final > _symbol_min_qty_value:
            ...
            if (_symbol_price_value * quantity_processed_final) > _symbol_min_notional_value:
                ...
                _response_tuple = (
                'OK', tuple((quantity_processed_final, quantity_pre_size_applied, quantity_post_size_applied)))
    ...
    def account_get_quantity_to_sell(self, p_type, p_size, p_price=None):
        ...
        try:
            ...
            _inputs_temp = f"||{type(p_size)},{p_size}|{type(_p_size_str)},{_p_size_str}|{type(_p_size_decimal)},{_p_size_decimal}"
            _response_tuple = ('NOK',
                               f"{utility.my_log('Exception', 'account_get_quantity_to_sell', _inputs + _inputs_temp, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        try:
            ...
            _inputs_temp = f"||{type(_p_size_decimal)},{_p_size_decimal}|{type(_division)},{_division}|{type(_multiplication)},{_multiplication}"
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_get_quantity_to_sell', _inputs, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        if p_type == 'market':
            ...
            try:
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Exception', 'account_get_quantity_to_sell', _inputs, traceback.format_exc(2))}")
                return (_response_tuple)
        else:
            _response_tuple = ('NOK',
                               f"{utility.my_log('Error', 'account_get_quantity_to_sell', _inputs, 'p_type unknown or p_price is None')}")
            return (_response_tuple)
    ...
    def account_set_symbol_leverage(self, p_leverage=50, p_symbol_input=None):
        ...
        try:
            ...
            if _response:
                ...
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'account_set_symbol_leverage', _inputs, '_response is None')}")
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_set_symbol_leverage', _inputs, traceback.format_exc(2))}")
    ...
    def account_create_order(self, p_type, p_side, p_size, p_limit=None, p_stop=None, p_price=None):
        ...
        if p_type == 'market':
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, p_type + ' unknown')}")
            return (_response_tuple)
        ...
        if p_side == 'sell':
            ...
            if self.wallet != 'futures':
                _quantity_calculated_result = self.account_get_quantity_to_buy(_what_fee, p_type, p_size,
                                                                               _how2get_qta2buy, p_limit)
                if _quantity_calculated_result[0] == 'OK':
                    _quantity_to_use = _quantity_calculated_result[1][0]
                    _quantity_pre_size_applied = _quantity_calculated_result[1][1]
                    _quantity_post_size_applied = _quantity_calculated_result[1][2]
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, p_side + ' unknown')}")
            return (_response_tuple)
        ...
        if p_type == 'market':
            ...
            try:
                ...
                _response_tuple = (
                'NOK', f"{utility.my_log('Exception', 'account_create_order', _inputs, traceback.format_exc())}")

        elif p_type == 'limit':

            if not p_limit:
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order Limit without limit price input')}")
                return (_response_tuple)
            ...
            if not p_limit:
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order Stop Limit without LIMIT input')}")
                return (_response_tuple)

            if not p_stop:
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order Stop Limit without STOP input')}")
                return (_response_tuple)
            ...
            if not p_limit:
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order OCO without LIMIT input')}")
                return (_response_tuple)

            if not p_stop:
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order OCO without STOP input')}")
                return (_response_tuple)

            if not p_price:
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, 'Order OCO without PRICE input')}")
                return (_response_tuple)
            ...
            try:
                ...
                if self.wallet == 'spot':
                    ...
                    _response_tuple = (
                    'NOK', f"{utility.my_log('Error', 'account_create_order', _inputs, 'no margin oco order exist')}")
    ...
    def account_get_open_orders(self, p_symbol_input=None):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_get_open_orders', _inputs, traceback.format_exc(2))}")
    ...
    def account_get_open_position_information(self, p_symbol_input=None):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'get_position_information', _inputs, traceback.format_exc(2))}")
    ...
    def account_cancel_order(self, p_symbol_input, p_orderid):
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_cancel_order', _inputs, traceback.format_exc())}")
    ...
    def account_convert_dust2bnb(self, p_symbol_input):
        ...
        if self.wallet == 'margin':
            _response_tuple = (
            'NOK', f"{utility.my_log('Error', 'account_convert_dust2bnb', _inputs, 'no margin convert 2 bnb exist')}")
            return (_response_tuple)
        ...
        try:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Exception', 'account_convert_dust2bnb', _inputs, traceback.format_exc(2))}")
            return (_response_tuple)
        ...
        if _result_raw:
            ...
            _response_tuple = (
            'NOK', f"{utility.my_log('Error', 'spot_convert_dust2bnb', _inputs, '_result_raw is None')}")
    ...
    def account_format_open_orders_result(self, p_result):
        ...
        for _dict_result in p_result:
            ...
            if _type == 'MARKET':  # MARKET TYPE ORDER
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Error', 'account_format_open_orders_result', _inputs, _type + ' _type unknown ')}")
                return (_response_tuple)
    ...
    def account_format_create_order_result(self, p_result, p_cancel=False, p_type=None):
        ...
        if p_result.get('status') == 'FILLED':
            ...
            try:
                ...
                _response_tuple = ('NOK',
                                   f"{utility.my_log('Exception', 'account_format_create_order_result', _inputs, traceback.format_exc())}")
                return (_response_tuple)
        ...
        if _type == 'MARKET':  # MARKET ORDER TYPE
            ...
            if p_result.get('side').upper() == 'BUY':
                ...
                _response_tuple = (
                'NOK', f"{utility.my_log('Error', 'account_format_create_order_result', _inputs, 'Side unknown')}")
                return (_response_tuple)
            ...
            _response_tuple = ('NOK',
                               f"{utility.my_log('Error', 'account_format_create_order_result', _inputs, _type + ' _type unknown')}")
            return (_response_tuple)
        ...
        for _dict_result in p_result:
            ...
            if _dict_result.get('positionAmt'):
                try:
                    ...
                    _response_tuple = ('NOK',
                                       f"{utility.my_log('Exception', 'account_format_open_position_result', _inputs + _inputs_temp, traceback.format_exc(2))}")
                    return (_response_tuple)
